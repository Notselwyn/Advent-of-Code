...
fs_entry* find_free_up(fs_entry* fs_root, int value, fs_entry* fs_best)
{
    for (int i=0; i < fs_root->children_len; i++)
    {
        ...
        if (child->size > value)
            if (child->size < fs_best->size)
                fs_best = child;
        
        fs_best = find_free_up(child, value, fs_best);
    }
...
int main()
{
    ...
    printf("root size: %lu\n", fs_root->size);
    unsigned long to_free = fs_root->size - 40000000;
    printf("need to free %ld (usage: %ld)\n", to_free, fs_root->size);

    fs_entry* fs_best = find_free_up(fs_root, to_free, fs_root);
    printf("best dir: '%s' (size: %lu)\n", fs_best->name, fs_best->size);
