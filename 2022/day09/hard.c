...
void tick_body(const int* head, int* tail, unsigned char steps_dir)
{
    ...
    unsigned int dist_sum = dist_th[0] + dist_th[1];
    
    if (dist_th[0] > 2 || dist_th[1] > 2)
    {
        printf("ERR @ tick(): dist too big (dy: %u, dx: %u)\n", dist_th[0], dist_th[1]);
        exit(1);
    }

    if (dist_sum > (dist_th[0] && dist_th[1] ? 2 : 1))  // check if the distance criteria is met
    {
        if (dist_th[0] >= 1)
...
int main()
{
    ...
    while (fgets(buf, sizeof(buf), file))
    {
        ...
        for (int i=0; i < steps_amount; i++)
        {
            ...
            for (int j=1; j < ROPE_LEN-1; j++)
            {
                tick_body(rope[j-1], rope[j], steps_dir);
                printf("%d (y: %d, x: %d), ", j, rope[j][0], rope[j][1]);   
            }
            tick_body(rope[ROPE_LEN-2], rope[ROPE_LEN-1], steps_dir);
            printf("tail: (y: %d, x: %d)\n", rope[ROPE_LEN-1][0], rope[ROPE_LEN-1][1]);   
            
            history_add(hst, rope[ROPE_LEN-1]);
        }
